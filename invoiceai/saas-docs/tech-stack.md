...
| **Verified blockers** | Missing OAuth callback route, unauthenticated AI generation endpoint, missing explicit AI rate limiting, failing build and test verification |
| **Billing target** | Paddle is the canonical provider; Stripe-era routes and docs remain migration work |
| **Token contract** | `E:\Yc_ai\invoiceai\tokens\design.tokens.ts` |
