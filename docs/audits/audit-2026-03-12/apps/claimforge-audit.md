# ClaimForge - Fresh Launch Readiness Audit

- Audit date: 2026-03-12
- Platform: web
- Canonical path: `E:/Yc_ai/claimforge`
- Product category: Whistleblower and claims evidence analysis
- Status: Completion Score: 45 / 100

## App Inventory

- Source files scanned: 181
- Approximate source lines scanned: 35096
- Document set loaded: 8 files from `E:/Yc_ai/saas-ideas/web-first/b2b/30-claimforge`
...
- Test result: failed -  ? Module.deleteClaim lib/actions/claims.ts:174:6
    172|     .delete()
    173|     .eq('id', id)
    174|     .eq('user_id', user.id);
       |      ^
    175| 
    176|   if (error) return { error: error.message };
 ? tests/actions/claims.test.ts:376:20
...
??????????????????????[13/13]?
 ...
 Test Files  4 failed | 1 passed (5)
   ...
   Start at  10:42:25
   Duration  3.92s (transform 190ms, setup 637ms, collect 144ms, tests 36ms, environment 4.55s, prepare 673ms)
...
The codebase for ClaimForge is organized around a route-driven web application with the following dominant module buckets:

- `app`: 84 source files
- `components`: 30 source files
- `lib`: 25 source files
...
- `E:/Yc_ai/claimforge/app/api/ai/generate/route.ts:1` - POST handler lacks a proven auth guard before OpenAI usage.
...
- `E:/Yc_ai/claimforge/app/api/ai/generate/route.ts:1` - POST handler streams OpenAI output without visible AI throttle.
...
- `E:/Yc_ai/claimforge/app/(dashboard)/analysis/page.tsx:166` - const t = setTimeout(() => setSummaryLoading(false), 1800);
- `E:/Yc_ai/claimforge/app/(dashboard)/documents/page.tsx:337` - setTimeout(() => {
- `E:/Yc_ai/claimforge/app/(dashboard)/documents/page.tsx:344` - setTimeout(() => {
- `E:/Yc_ai/claimforge/app/(dashboard)/documents/page.tsx:353` - setTimeout(() => {
- `E:/Yc_ai/claimforge/app/(dashboard)/settings/referral/page.tsx:19` - setTimeout(() => setCopied(false), 2000);
- `E:/Yc_ai/claimforge/app/onboarding/step-4/page.tsx:48` - await new Promise(r => setTimeout(r, 400));
- `E:/Yc_ai/claimforge/app/onboarding/step-5/page.tsx:26` - await new Promise(r => setTimeout(r, 500));
- `E:/Yc_ai/claimforge/components/CommandPalette.tsx:210` - debounceRef.current = setTimeout(async () => {
...
- `E:/Yc_ai/claimforge/components/NotificationCenter.tsx:67` - .select('*')
- `E:/Yc_ai/claimforge/lib/actions/account.ts:70` - .select('*')
- `E:/Yc_ai/claimforge/lib/actions/analysis.ts:28` - .select('*')
- `E:/Yc_ai/claimforge/lib/actions/analysis.ts:43` - .select('*')
- `E:/Yc_ai/claimforge/lib/actions/analysis.ts:75` - .select('*')
...
The recorded runtime test or typecheck failed:  ? Module.deleteClaim lib/actions/claims.ts:174:6
    172|     .delete()
    173|     .eq('id', id)
    174|     .eq('user_id', user.id);
       |      ^
...
Benchmark set for ClaimForge:
- [NAVEX](https://www.navex.com/)
- [Case IQ](https://caseiq.com/)
- [EQS Integrity Line](https://www.integrityline.com/)
