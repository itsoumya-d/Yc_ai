# StoryThread -- Database Schema

## Entity Relationship Summary

```
users 1--N stories (as author)
users 1--N reading_lists
users 1--N followers (as follower or followed)
users 1--1 subscriptions (platform subscription)
users 1--N writer_subscriptions (subscribe to a writer)
users 1--N comments
users 1--N reactions
...
stories 1--N worlds
stories 1--N collaborators (join: users <-> stories)
stories 1--N comments
stories 1--N reading_list_items

worlds 1--N world_elements

chapters 1--N comments
chapters 1--N reactions
chapters 1--N ai_generations

reading_lists 1--N reading_list_items
```
...
### Writer Subscriptions (reader pays writer)
...
-- Characters: viewable by story author and collaborators
...
CREATE TRIGGER update_world_elements_updated_at BEFORE UPDATE ON world_elements FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_writer_subs_updated_at BEFORE UPDATE ON writer_subscriptions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER audit_chapters AFTER INSERT OR UPDATE OR DELETE ON chapters
  FOR EACH ROW EXECUTE FUNCTION audit_log_trigger();
CREATE TRIGGER audit_writer_subscriptions AFTER INSERT OR UPDATE OR DELETE ON writer_subscriptions
  FOR EACH ROW EXECUTE FUNCTION audit_log_trigger();
```
...
export interface Story {
  ...
  status: 'draft' | 'ongoing' | 'completed' | 'hiatus' | 'archived';
  total_words: number;
  ...
  ai_genre_settings: Record<string, unknown>;
...
export interface WorldElement {
  ...
  name: string;
  element_type: 'location' | 'lore' | 'rule' | 'event' | 'item' | 'faction';
  description: string | null;
...
export interface Collaborator {
  ...
  story_id: string;
  user_id: string;
  role: 'owner' | 'writer' | 'editor' | 'viewer';
...
export interface Reaction {
  ...
  reaction_type: 'like' | 'love' | 'surprised' | 'sad' | 'laughing';
  created_at: string;
}
...
export interface WriterSubscription {
  id: string;
...
export interface Subscription {
  id: string;
  user_id: string;
  plan: 'free' | 'writer_pro' | 'writer_unlimited';
...
export interface Notification {
  id: string;
  user_id: string;
  type: 'new_chapter' | 'comment' | 'follow' | 'milestone' | 'tip' | 'subscription' | 'system';
  title: string;
  body: string | null;
...
```sql
-- Default reading lists for new users
-- Created programmatically on user signup: "To Read" and "Favorites"

-- Sample genres for discovery
INSERT INTO stories (author_id, title, slug, description, genre, status, content_rating) VALUES
  -- Seeded via admin panel or content team

-- Genre categories (stored as application constant, not DB)
-- Fantasy, Sci-Fi, Romance, Mystery, Horror, Literary Fiction, Fan Fiction,
-- Thriller, Historical, Comedy, Young Adult, Dystopian, Adventure

-- Sample notification templates are handled at application level
```
...
migrations/
  ...
  003_create_stories.sql
  004_create_chapters.sql
  005_create_characters.sql
  006_create_worlds.sql
  007_create_world_elements.sql
  008_create_collaborators.sql
  009_create_comments.sql
  010_create_reactions.sql
  011_create_reading_lists.sql
  012_create_reading_list_items.sql
  013_create_reading_progress.sql
  014_create_followers.sql
  015_create_writer_subscriptions.sql
  016_create_ai_generations.sql
  017_create_subscriptions.sql
  018_create_notifications.sql
  019_create_audit_log.sql
  020_create_indexes.sql
  021_create_rls_policies.sql
  022_create_functions_and_triggers.sql
  023_seed_data.sql
```
...
2. Core tables: users first, then stories, chapters, etc.
3. Join and relationship tables (collaborators, followers, reading_list_items)
4. Feature tables (ai_generations, notifications)
5. Audit log table
6. All indexes
7. RLS policies
8. Functions and triggers (updated_at, search vectors, audit)
9. Seed data
