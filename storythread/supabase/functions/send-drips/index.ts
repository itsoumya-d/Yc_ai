...
const COLOR = '#7C3AED';
const APP_URL = 'https://storythread.app';
...
const SEQUENCES: Record<DripStep, { subject: string; preheader: string; headline: string; body: string; cta: string; ctaUrl: string }> = {
  day1: {
    ...
    headline: 'Get the most from StoryThread',
  ...
  day7: {
    ...
    headline: 'Your first week as a StoryThread author',
...
async function sendEmail(to: string, subject: string, html: string): Promise<void> {
  ...
  const res = await fetch('https://api.sendgrid.com/v3/mail/send', {
    ...
    body: JSON.stringify({
      ...
      from: { email: Deno.env.get('FROM_EMAIL') ?? 'noreply@storythread.app', name: Deno.env.get('FROM_NAME') ?? BRAND },
      subject,
