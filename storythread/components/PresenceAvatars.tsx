  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="flex items-center gap-1.5">
        <span className="text-xs text-[var(--muted-foreground)] select-none">Also writing:</span>
        <div className="flex -space-x-2">
          ...
          {overflow > 0 && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div
                  aria-label={`${overflow} more writer${overflow > 1 ? 's' : ''}`}
              ...
              <TooltipContent>{overflow} more writer{overflow > 1 ? 's' : ''}</TooltipContent>
            </Tooltip>
          )}
        </div>
      </div>
