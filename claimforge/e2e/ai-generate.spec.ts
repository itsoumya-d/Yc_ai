...
/**
 * AI Feature tests for ClaimForge — AI Fraud Detection.
 * Verifies AI endpoints return proper responses and UI handles loading states.
 */
test.describe('AI Features — ClaimForge', () => {
  test('AI endpoint returns 401 (not 500) when unauthenticated', async ({ page }) => {
    const res = await page.request.post('/api/fraud/analyze', {
      data: {"caseId":"test-case-123"},
  ...
  test('AI loading state is handled gracefully in UI (mock)', async ({ page }) => {
    ...
    await page.route('/api/fraud/analyze', async (route) => {
      // Delay response by 100ms to test loading state
      await new Promise(r => setTimeout(r, 100));
      route.fulfill({
        status: 200,
        body: JSON.stringify({"nodes":[],"edges":[],"stats":{"highRiskCount":0}}),
      });
      resolveRequest(null);
    });
  ...
  test('AI error is handled gracefully (mock 500)', async ({ page }) => {
    ...
    await page.route('/api/fraud/analyze', route => {
  ...
  test('OpenAI/AI routes respect rate limiting', async ({ page }) => {
    ...
    const requests = await Promise.all(
      Array.from({ length: 3 }, () =>
        page.request.post('/api/fraud/analyze', {
          data: {"caseId":"test-case-123"},
          headers: { 'Content-Type': 'application/json' },
        })
      )
