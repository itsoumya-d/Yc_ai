'use client';

import { useState, useTransition } from 'react';
import { PageHeader } from '@/components/layout/page-header';
import { cn, formatDate, getRoleLabel } from '@/lib/utils';
import {
  User,
  Building2,
  Shield,
  Bell,
  Key,
  Database,
  FileText,
  Palette,
  CheckCircle2,
} from 'lucide-react';
import type { User as UserType, Organization, AuditLogEntry } from '@/types/database';
import {
  updateUserProfile,
  updateOrganization,
} from '@/lib/actions/settings';

const settingsNav = [
  { id: 'profile', icon: User, label: 'Profile' },
  { id: 'organization', icon: Building2, label: 'Organization' },
  { id: 'security', icon: Shield, label: 'Security' },
  { id: 'notifications', icon: Bell, label: 'Notifications' },
  { id: 'api', icon: Key, label: 'API Keys' },
  { id: 'data', icon: Database, label: 'Data & Storage' },
  { id: 'compliance', icon: FileText, label: 'Compliance' },
  { id: 'appearance', icon: Palette, label: 'Appearance' },
];

interface SettingsViewProps {
  profile: UserType | null;
  organization: Organization | null;
  auditLog: AuditLogEntry[];
}

export function SettingsView({ profile, organization, auditLog }: SettingsViewProps) {
  const [activeSection, setActiveSection] = useState('profile');
  const [isPending, startTransition] = useTransition();
  const [saveSuccess, setSaveSuccess] = useState<string | null>(null);
  const [saveError, setSaveError] = useState<string | null>(null);

  const [profileForm, setProfileForm] = useState({
    full_name: profile?.full_name ?? '',
    title: (profile as (UserType & { title?: string }) | null)?.title ?? '',
  });

  const [orgForm, setOrgForm] = useState({
    name: organization?.name ?? '',
    domain: organization?.domain ?? '',
  });

  function showResult(success: boolean, section: string) {
    if (success) {
      setSaveSuccess(`${section} saved successfully`);
      setSaveError(null);
      setTimeout(() => setSaveSuccess(null), 3000);
    }
  }

  function handleSaveProfile() {
    startTransition(async () => {
      const result = await updateUserProfile({
        full_name: profileForm.full_name,
      });
      if (result.error) {
        setSaveError(result.error);
      } else {
        showResult(true, 'Profile');
      }
    });
  }

  function handleSaveOrg() {
    startTransition(async () => {
      const result = await updateOrganization({
        name: orgForm.name,
        domain: orgForm.domain,
      });
      if (result.error) {
        setSaveError(result.error);
      } else {
        showResult(true, 'Organization');
      }
    });
  }

  return (
    <div className="flex h-full flex-col">
      <PageHeader title="Settings" subtitle="Manage your account and organization" />

      <div className="flex flex-1 overflow-hidden">
        {/* Settings Nav */}
        <div className="w-52 shrink-0 border-r border-border-default p-3">
          <nav className="space-y-0.5">
            {settingsNav.map((item) => (
              <button
                key={item.id}
                onClick={() => setActiveSection(item.id)}
                className={cn(
                  'flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors',
                  activeSection === item.id
                    ? 'bg-primary-muted text-primary-light'
                    : 'text-text-secondary hover:bg-bg-surface-raised hover:text-text-primary'
                )}
              >
                <item.icon className="h-4 w-4" />
                {item.label}
              </button>
            ))}
          </nav>
        </div>

        {/* Settings Content */}
        <div className="flex-1 overflow-auto p-6">
          {/* Status messages */}
          {saveSuccess && (
            <div className="mb-4 flex items-center gap-2 rounded-lg bg-verified-green-muted px-4 py-2 text-sm text-verified-green">
              <CheckCircle2 className="h-4 w-4" />
              {saveSuccess}
            </div>
          )}
          {saveError && (
            <div className="mb-4 rounded-lg bg-fraud-red-muted px-4 py-2 text-sm text-fraud-red">
              {saveError}
            </div>
          )}

          {/* Profile */}
          {activeSection === 'profile' && (
            <div className="max-w-2xl space-y-6">
              <h2 className="legal-heading text-base text-text-primary">Profile Settings</h2>
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Full Name
                  </label>
                  <input
                    type="text"
                    value={profileForm.full_name}
                    onChange={(e) =>
                      setProfileForm((f) => ({ ...f, full_name: e.target.value }))
                    }
                    className="h-9 w-full rounded-lg border border-border-default bg-bg-surface-raised px-3 text-sm text-text-primary focus:border-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Email
                  </label>
                  <input
                    type="email"
                    defaultValue={profile?.email ?? ''}
                    disabled
                    className="h-9 w-full rounded-lg border border-border-default bg-bg-surface px-3 text-sm text-text-tertiary"
                  />
                  <p className="mt-1 text-[10px] text-text-tertiary">
                    Email is managed by authentication provider
                  </p>
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Role
                  </label>
                  <div className="h-9 rounded-lg border border-border-default bg-bg-surface px-3 flex items-center text-sm text-text-tertiary">
                    {profile?.role ? getRoleLabel(profile.role) : 'Unknown'}
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Member Since
                  </label>
                  <div className="h-9 rounded-lg border border-border-default bg-bg-surface px-3 flex items-center text-sm text-text-tertiary">
                    {profile?.created_at ? formatDate(profile.created_at) : '—'}
                  </div>
                </div>
                <button
                  onClick={handleSaveProfile}
                  disabled={isPending}
                  className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-text-on-color transition-colors hover:bg-primary-hover disabled:opacity-50"
                >
                  {isPending ? 'Saving...' : 'Save Changes'}
                </button>
              </div>
            </div>
          )}

          {/* Organization */}
          {activeSection === 'organization' && (
            <div className="max-w-2xl space-y-6">
              <h2 className="legal-heading text-base text-text-primary">Organization Settings</h2>
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Organization Name
                  </label>
                  <input
                    type="text"
                    value={orgForm.name}
                    onChange={(e) => setOrgForm((f) => ({ ...f, name: e.target.value }))}
                    className="h-9 w-full rounded-lg border border-border-default bg-bg-surface-raised px-3 text-sm text-text-primary focus:border-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Domain
                  </label>
                  <input
                    type="text"
                    value={orgForm.domain}
                    onChange={(e) => setOrgForm((f) => ({ ...f, domain: e.target.value }))}
                    placeholder="e.g. yourfirm.com"
                    className="h-9 w-full rounded-lg border border-border-default bg-bg-surface-raised px-3 text-sm text-text-primary placeholder:text-text-tertiary focus:border-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Organization ID
                  </label>
                  <div className="h-9 rounded-lg border border-border-default bg-bg-surface px-3 flex items-center font-mono text-xs text-text-tertiary">
                    {organization?.id ?? '—'}
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-text-secondary">
                    Created
                  </label>
                  <div className="h-9 rounded-lg border border-border-default bg-bg-surface px-3 flex items-center text-sm text-text-tertiary">
                    {organization?.created_at ? formatDate(organization.created_at) : '—'}
                  </div>
                </div>
                <button
                  onClick={handleSaveOrg}
                  disabled={isPending}
                  className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-text-on-color transition-colors hover:bg-primary-hover disabled:opacity-50"
                >
                  {isPending ? 'Saving...' : 'Save Changes'}
                </button>
              </div>
            </div>
          )}

          {/* Security */}
          {activeSection === 'security' && (
            <div className="max-w-2xl space-y-6">
              <h2 className="legal-heading text-base text-text-primary">Security Settings</h2>

              <div className="rounded-xl border border-border-default bg-bg-surface p-5 space-y-4">
                <h3 className="text-sm font-medium text-text-primary">
                  Two-Factor Authentication
                </h3>
                <p className="text-xs text-text-secondary">
                  Add an extra layer of security to your account with 2FA.
                </p>
                <button className="rounded-lg border border-primary px-4 py-2 text-sm font-medium text-primary-light transition-colors hover:bg-primary-muted">
                  Enable 2FA
                </button>
              </div>

              <div className="rounded-xl border border-border-default bg-bg-surface p-5 space-y-4">
                <h3 className="text-sm font-medium text-text-primary">Audit Log</h3>
                <p className="text-xs text-text-secondary">
                  All actions are logged for compliance and security review.
                </p>
                <div className="space-y-2">
                  {auditLog.length === 0 ? (
                    <p className="text-xs text-text-tertiary">No audit log entries found</p>
                  ) : (
                    auditLog.slice(0, 10).map((entry) => (
                      <div
                        key={entry.id}
                        className="flex items-center justify-between rounded-lg border border-border-muted px-3 py-2"
                      >
                        <div>
                          <span className="text-xs text-text-secondary">{entry.action}</span>
                          {entry.resource_type && (
                            <span className="ml-2 text-[10px] text-text-tertiary capitalize">
                              {entry.resource_type}
                            </span>
                          )}
                        </div>
                        <div className="text-right">
                          <div className="text-[10px] text-text-tertiary">
                            {entry.user_name}
                          </div>
                          <div className="text-[10px] text-text-tertiary">
                            {formatDate(entry.created_at)}
                          </div>
                        </div>
                      </div>
                    ))
                  )}
                </div>
              </div>
            </div>
          )}

          {/* API Keys */}
          {activeSection === 'api' && (
            <div className="max-w-2xl space-y-6">
              <h2 className="legal-heading text-base text-text-primary">API Keys</h2>

              <div className="rounded-xl border border-border-default bg-bg-surface p-5 space-y-4">
                <h3 className="text-sm font-medium text-text-primary">OpenAI API Key</h3>
                <p className="text-xs text-text-secondary">
                  Used for document analysis, entity extraction, and fraud pattern detection.
                </p>
                <div className="flex items-center gap-2">
                  <input
                    type="password"
                    placeholder="sk-proj-..."
                    className="h-9 flex-1 rounded-lg border border-border-default bg-bg-surface-raised px-3 font-mono text-sm text-text-primary focus:border-primary focus:outline-none"
                  />
                  <button className="rounded-lg border border-border-default px-3 py-2 text-xs text-text-secondary transition-colors hover:bg-bg-surface-raised">
                    Update
                  </button>
                </div>
                <p className="text-[10px] text-text-tertiary">
                  API keys are configured via environment variables for security.
                </p>
              </div>

              <div className="rounded-xl border border-border-default bg-bg-surface p-5 space-y-4">
                <h3 className="text-sm font-medium text-text-primary">Google Cloud Vision</h3>
                <p className="text-xs text-text-secondary">
                  Used for OCR processing of scanned documents and images.
                </p>
                <div className="flex items-center gap-2">
                  <input
                    type="password"
                    placeholder="AIzaSy..."
                    className="h-9 flex-1 rounded-lg border border-border-default bg-bg-surface-raised px-3 font-mono text-sm text-text-primary focus:border-primary focus:outline-none"
                  />
                  <button className="rounded-lg border border-border-default px-3 py-2 text-xs text-text-secondary transition-colors hover:bg-bg-surface-raised">
                    Update
                  </button>
                </div>
              </div>

              <div className="rounded-lg border border-accent bg-accent-muted p-4">
                <div className="flex items-start gap-2">
                  <Shield className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
                  <div>
                    <div className="text-xs font-medium text-text-primary">Security Notice</div>
                    <p className="mt-0.5 text-xs text-text-secondary">
                      API keys are encrypted at rest and never logged. Keys are only accessible to
                      organization administrators.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          )}

          {/* Placeholder for remaining sections */}
          {!['profile', 'organization', 'security', 'api'].includes(activeSection) && (
            <div className="max-w-2xl">
              <h2 className="legal-heading text-base text-text-primary">
                {settingsNav.find((s) => s.id === activeSection)?.label}
              </h2>
              <p className="mt-2 text-sm text-text-secondary">
                Settings for{' '}
                {settingsNav.find((s) => s.id === activeSection)?.label?.toLowerCase()}{' '}
                configuration. Additional options coming soon.
              </p>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}

