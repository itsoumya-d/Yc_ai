...
function ActionButton({ label, onClick, href, icon, variant }: ActionDef & { variant: 'primary' | 'secondary' }) {
  const cls = cn(
    ...
    variant === 'primary'
      ? 'bg-primary text-text-on-color hover:bg-primary-hover shadow-sm'
      : 'border border-border-default bg-bg-surface text-text-secondary hover:bg-bg-surface-raised'
  );
  const content = (
    <>
  ...
  return (
    ...
    >
        ...
        >
          {IconComponent ? (
            <div className={cn('flex items-center justify-center rounded-2xl', s.iconWrap, iconColor)}>
              <IconComponent className={cn(s.iconSize, 'text-primary-light')} />
            </div>
          ) : (
            <motion.span
      ...
      <motion.h3
        ...
        className={cn('font-semibold text-text-primary mb-2', s.title)}
      >
        {title}
      </motion.h3>
