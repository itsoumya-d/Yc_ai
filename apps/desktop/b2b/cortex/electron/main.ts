...
ipcMain.handle('get-schema', async () => {
  try {
    ...
    return tables.map(({ name }) => {
      // NOTE: PRAGMA table_info requires parameter binding or careful escaping.
      // With better-sqlite3, parameter binding for pragma values works if we use pragma_table_info instead of PRAGMA table_info,
      // but PRAGMA table_info("name") is also safe if we properly escape the name.
