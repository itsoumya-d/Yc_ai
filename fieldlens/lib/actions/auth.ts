...
export async function signUp(formData: FormData): Promise<ActionResult<Profile>> {
  const raw = {
    email: formData.get('email') as string,
    password: formData.get('password') as string,
    trade: (formData.get('trade') as string) || 'general',
  };
...
export async function getUser(): Promise<ActionResult<Profile>> {
  try {
    const supabase = await createClient();
