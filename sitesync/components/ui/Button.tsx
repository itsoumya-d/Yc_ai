...
export function Button({ label, onPress, loading = false, disabled = false, variant = "primary", size = "md", style, textStyle }: ButtonProps) {
  ...
  return (
    ...
    >
      {loading ? (
        <ActivityIndicator size="small" color={variant === "secondary" || variant === "ghost" ? "#d97706" : "#fff"} />
      ) : (
...
const styles = StyleSheet.create({
  ...
  secondary: { backgroundColor: "#fff", borderWidth: 1.5, borderColor: "#d97706" },
  danger: { backgroundColor: "#dc2626" },
  ghost: { backgroundColor: "transparent" },
  disabled: { opacity: 0.5 },
  text: { color: "#fff", fontWeight: "700", fontSize: 15 },
  textSecondary: { color: "#d97706" },
  textGhost: { color: "#d97706" },
  textSm: { fontSize: 13 },
});
