...
const navItems: NavItem[] = [
  ...
  { label: 'Scan Bill', href: '/scanner', icon: <ScanLine className="h-5 w-5" /> },
  { label: 'Disputes', href: '/disputes', icon: <Gavel className="h-5 w-5" /> },
  { label: 'Bank Monitor', href: '/bank', icon: <Landmark className="h-5 w-5" /> },
  { label: 'Savings', href: '/savings', icon: <PiggyBank className="h-5 w-5" /> },
    ...
    <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-white border-r border-border">
          ...
          return (
            <Link
              ...
              className={`
                ...
                ${
                  isActive
                    ? 'bg-champion-50 text-champion-600'
      ...
      <div className="border-t border-border px-4 py-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-champion-100 text-champion-600">
            <span className="text-sm font-semibold font-heading">CB</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-text-primary truncate">User</p>
            <p className="text-xs text-text-muted truncate">user@claimback.app</p>
          </div>
        </div>
      </div>
    </aside>
          ...
          return (
            <Link
              ...
              className={`
                ...
                transition-colors duration-150
                ${isActive ? 'text-champion-600' : 'text-text-muted'}
              `}
            >
              {item.icon}
