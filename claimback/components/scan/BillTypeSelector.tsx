import { View, Text, TouchableOpacity, StyleSheet, ScrollView } from 'react-native';
import type { BillType } from '../../types';

interface BillTypeSelectorProps {
  selected: BillType;
  onSelect: (type: BillType) => void;
}

const BILL_TYPES: { type: BillType; label: string; icon: string }[] = [
  { type: 'medical', label: 'Medical', icon: '🏥' },
  { type: 'bank', label: 'Bank', icon: '🏦' },
  { type: 'utility', label: 'Utility', icon: '💡' },
  { type: 'insurance', label: 'Insurance', icon: '🛡️' },
  { type: 'telecom', label: 'Telecom', icon: '📱' },
  { type: 'other', label: 'Other', icon: '📄' },
];

export function BillTypeSelector({ selected, onSelect }: BillTypeSelectorProps) {
  return (
    <ScrollView
      horizontal
      showsHorizontalScrollIndicator={false}
      contentContainerStyle={styles.container}
    >
      {BILL_TYPES.map(({ type, label, icon }) => (
        <TouchableOpacity
          key={type}
          style={[styles.chip, selected === type && styles.chipSelected]}
          onPress={() => onSelect(type)}
          activeOpacity={0.7}
        >
          <Text style={styles.chipIcon}>{icon}</Text>
          <Text style={[styles.chipLabel, selected === type && styles.chipLabelSelected]}>
            {label}
          </Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
}

const styles = StyleSheet.create({
  container: { gap: 8, paddingVertical: 4, marginBottom: 4 },
  chip: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
    paddingHorizontal: 14,
    paddingVertical: 9,
    borderRadius: 24,
    backgroundColor: '#ffffff',
    borderWidth: 1.5,
    borderColor: '#e2e8f0',
  },
  chipSelected: {
    backgroundColor: '#16a34a',
    borderColor: '#16a34a',
  },
  chipIcon: { fontSize: 16 },
  chipLabel: { fontSize: 13, fontWeight: '500', color: '#64748b' },
  chipLabelSelected: { color: '#ffffff' },
});
