...
/**
 * AI Feature tests for BoardBrief — AI Board Pack Generation.
 * Verifies AI endpoints return proper responses and UI handles loading states.
 */
test.describe('AI Features — BoardBrief', () => {
  test('AI endpoint returns 401 (not 500) when unauthenticated', async ({ page }) => {
    const res = await page.request.post('/api/ai/board-pack', {
      data: {"meetingId":"test-meeting","sections":["financials","kpis"]},
  ...
  test('AI loading state is handled gracefully in UI (mock)', async ({ page }) => {
    ...
    await page.route('/api/ai/board-pack', async (route) => {
      // Delay response by 100ms to test loading state
      await new Promise(r => setTimeout(r, 100));
      route.fulfill({
        status: 200,
        body: JSON.stringify({"slides":[{"title":"Q1 Performance","content":"..."}]}),
      });
      resolveRequest(null);
    });
  ...
  test('AI error is handled gracefully (mock 500)', async ({ page }) => {
    ...
    await page.route('/api/ai/board-pack', route => {
  ...
  test('OpenAI/AI routes respect rate limiting', async ({ page }) => {
    ...
    const requests = await Promise.all(
      Array.from({ length: 3 }, () =>
        page.request.post('/api/ai/board-pack', {
          data: {"meetingId":"test-meeting","sections":["financials","kpis"]},
          headers: { 'Content-Type': 'application/json' },
        })
      )
