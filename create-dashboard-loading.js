/**
 * Creates app/(dashboard)/loading.tsx for all 10 web apps.
 * This acts as the top-level Suspense fallback for any dashboard route
 * that doesn't have its own loading.tsx, fixing LCP/CLS.
 */
const fs = require('fs');
...
const loadingContent = `export default function DashboardLoading() {
  return (
...
webApps.forEach(app => {
  const dashboardDir = `E:/Yc_ai/${app}/app/(dashboard)`;
  if (!fs.existsSync(dashboardDir)) { console.log('SKIP:', app); return; }

  const loadingPath = `${dashboardDir}/loading.tsx`;
  if (fs.existsSync(loadingPath)) {
    console.log('Already exists:', app);
    return;
  }

  fs.writeFileSync(loadingPath, loadingContent, 'utf8');
  console.log('Created loading.tsx:', app);
});
