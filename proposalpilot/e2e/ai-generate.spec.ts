...
/**
 * AI Feature tests for ProposalPilot — AI Proposal Generation.
 * Verifies AI endpoints return proper responses and UI handles loading states.
 */
test.describe('AI Features — ProposalPilot', () => {
  test('AI endpoint returns 401 (not 500) when unauthenticated', async ({ page }) => {
    const res = await page.request.post('/api/ai/generate-proposal', {
      data: {"clientName":"Acme Corp","projectType":"web development","budget":50000},
  ...
  test('AI loading state is handled gracefully in UI (mock)', async ({ page }) => {
    ...
    await page.route('/api/ai/generate-proposal', async (route) => {
      // Delay response by 100ms to test loading state
      await new Promise(r => setTimeout(r, 100));
      route.fulfill({
        status: 200,
        body: JSON.stringify({"sections":[{"title":"Executive Summary","content":"..."}]}),
      });
      resolveRequest(null);
    });
  ...
  test('AI error is handled gracefully (mock 500)', async ({ page }) => {
    ...
    await page.route('/api/ai/generate-proposal', route => {
  ...
  test('OpenAI/AI routes respect rate limiting', async ({ page }) => {
    ...
    const requests = await Promise.all(
      Array.from({ length: 3 }, () =>
        page.request.post('/api/ai/generate-proposal', {
          data: {"clientName":"Acme Corp","projectType":"web development","budget":50000},
          headers: { 'Content-Type': 'application/json' },
        })
      )
