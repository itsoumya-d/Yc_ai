...
function ActionButton({ label, onClick, href, icon, variant }: ActionDef & { variant: 'primary' | 'secondary' }) {
  const cls = cn(
    'inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-all active:scale-95',
    variant === 'primary'
      ? 'bg-[var(--primary,#6366F1)] text-white hover:opacity-90 shadow-sm'
  ...
  return <button onClick={onClick} type="button" className={cls}>{content}</button>;
}

export function EmptyState({
  icon,
  iconColor = 'bg-[var(--primary,#6366F1)]/10',
  title,
  description,
  action,
  secondaryAction,
  ...
  return (
    ...
    >
        ...
        >
          {IconComponent ? (
            <div className={cn('flex items-center justify-center rounded-2xl', s.iconWrap, iconColor)}>
              <IconComponent className={cn(s.iconSize, 'text-[var(--primary,#6366F1)]')} />
            </div>
          ) : (
            <motion.span
      ...
      <motion.h3
        ...
        className={cn('font-semibold text-[var(--foreground)] mb-2', s.title)}
      >
        {title}
      </motion.h3>
      ...
      {description && (
        <motion.p
          ...
          className={cn(
            'text-[var(--muted-foreground,#64748B)] max-w-sm leading-relaxed',
            s.desc,
