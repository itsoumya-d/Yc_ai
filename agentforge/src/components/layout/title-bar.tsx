...
export function TitleBar() {
  const { currentAgentName, currentView } = useAppStore();
  ...
  return (
    <div className="titlebar-drag flex h-10 items-center justify-between border-b border-border-default bg-bg-root px-4">
      ...
      <div className="flex items-center gap-2 pl-16">
        <span className="text-gradient-brand font-heading text-sm font-bold">
          AgentForge
        </span>
        {currentAgentName && (
          <>
      ...
      <div className="titlebar-no-drag flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-2 rounded-md border border-border-default bg-bg-surface px-3 py-1 text-xs text-text-tertiary transition-colors hover:border-border-emphasis hover:text-text-secondary"
        >
