...
/**
 * Core feature tests for PetOS — Pet Health Records.
 * Tests the primary user flow for creating/accessing the core entity.
 */
test.describe('Core Feature: Pet Health Records — PetOS', () => {
  test('Pets page loads or redirects correctly', async ({ page }) => {
  ...
  test('create pet form is accessible when authenticated (stub)', async ({ page }) => {
    ...
    const res = await page.goto('/pets');
    expect(res?.status()).not.toBe(500);
  });

  test('pet list page has correct structure (unauthenticated)', async ({ page }) => {
  ...
  test('API routes for pet return proper auth error when unauthenticated', async ({ page }) => {
    // Direct API call without auth should return 401 or 302, not 500
    const res = await page.request.get('/api/pets');
    expect([200, 301, 302, 307, 308, 401, 403]).toContain(res.status());
  });

  test('landing page showcases Pet Health Records value proposition', async ({ page }) => {
