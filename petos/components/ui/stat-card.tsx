...
export function StatCard({ title, value, icon, trend, className, index = 0, animateValue = false }: StatCardProps) {
  return (
    <motion.div
      ...
      <div className="flex items-center justify-between">
        ...
        {icon && (
          <motion.div
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-50 text-brand-600"
          >
            {icon}
          </motion.div>
        )}
      ...
      <div className="mt-2 flex items-baseline gap-2">
        ...
        {trend && (
          <motion.span
            ...
            transition={{ duration: 0.3, delay: index * 0.08 + 0.3, type: 'spring' }}
            className={cn(
