...
import { importHealthRecordsFromCSV } from '@/lib/actions/csv-import';
...
export function CSVImport({ onSuccess }: CSVImportProps) {
  ...
  function handleImport() {
    ...
    startTransition(async () => {
      const res = await importHealthRecordsFromCSV(rawRows);
      setResult(res);
      setPreview([]);
      setHeaders([]);
      setFileName('');
  ...
  return (
    <div className="rounded-xl border border-[var(--border,#E2E8F0)] p-6 space-y-4 bg-[var(--card,#fff)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="font-semibold text-[var(--foreground,#0F172A)]">Import Health Records from CSV</h3>
          <p className="text-sm text-[var(--muted-foreground,#64748B)] mt-0.5">
            Expected columns: <code className="text-xs bg-[var(--muted,#F1F5F9)] px-1 py-0.5 rounded">pet_name, record_type, date, notes, vet_name</code>
          </p>
          <p className="text-xs text-[var(--muted-foreground,#64748B)] mt-1">
            record_type: vaccination / checkup / medication / surgery / vet_visit / dental / lab_work
          </p>
        </div>
        <button
      ...
      {preview.length > 0 && (
        <div className="space-y-3">
          ...
          >
            {isPending ? 'Importing...' : `Import ${rawRows.length} Record${rawRows.length !== 1 ? 's' : ''}`}
          </button>
        </div>
      )}
      ...
      {result && (
        <div className={`rounded-lg p-3 text-sm ${result.errors.length === 0 ? 'bg-emerald-50 border border-emerald-200 text-emerald-800' : 'bg-amber-50 border border-amber-200 text-amber-800'}`}>
          {result.errors.length === 0
            ...
            : `Imported ${result.imported} record${result.imported !== 1 ? 's' : ''} — ${result.errors.length} row${result.errors.length !== 1 ? 's' : ''} skipped`}
          {result.errors.length > 0 && (
